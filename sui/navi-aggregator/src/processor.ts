...
async function swapEventHandler(
  ...
  if (processedTxs.has(txDigest)) {
    console.log(
      `Transaction ${txDigest} has already been processed. Skipping.`
    );
    return;
  }
  ...
  ctx.eventLogger.emit("swapEvent", {
    user: String(event.sender),
    from: String(event.type_arguments[0]),
    fromSymbol: String(updateSymbol(fromInfo, event.type_arguments[0])),
    target: String(event.type_arguments[1]),
    targetSymbol: String(updateSymbol(toInfo, event.type_arguments[1])),
    amount_in: String(event.data_decoded.amount_in),
    amount_in_number:
      Number(event.data_decoded.amount_in) / Math.pow(10, fromInfo.decimal) ||
      0,
    amount_in_usd: Number(fromValue) || 0,
    amount_out: String(event.data_decoded.amount_out),
    amount_out_number:
      Number(event.data_decoded.amount_out) / Math.pow(10, toInfo.decimal) || 0,
    amount_out_usd: Number(toValue) || 0,
    min_amount_out: String(event.data_decoded.min_amount_out),
    referral_code: String(event.data_decoded.referral_code),
  });
          ...
          for (let j = 0; j < events.length; j++) {
            ...
            if (
              ...
              ctx.eventLogger.emit("sponsorTX", {
                suiGas: Math.abs(Number(amount)) || 0,
                coinType: String(coinType),
                owner: String(ownerAddress),
                txHash: String(ctx.transaction.digest),
                user: String(txEvent.sender),
                eventType: String(txEvent.type),
                eventId: String(txEvent.id),
              });
            }
          }
        }
...
async function OnBehalfOfExSwapWithReferral(
  ...
  ctx.eventLogger.emit("swapReferralEvent", {
    ...
    fromCoinPrice: String(fromCoinPrice),
    fromCoinAmount: String(fromCoinAmount),
    toCoinPrice: String(toCoinPrice),
    toCoinAmount: String(toCoinAmount),
    rewardsAmount: String(rewardsAmount),
    fromCoinPriceNumber: Number(fromCoinPriceNumber) || 0,
    fromCoinAmountNumber: Number(fromCoinAmountNumber) || 0,
    fromValueInUSD: Number(fromValueInUSD) || 0,
    toCoinPriceNumber: Number(toCoinPriceNumber) || 0,
    toCoinAmountNumber: Number(toCoinAmountNumber) || 0,
    toValueInUSD: Number(toValueInUSD) || 0,
    rewardsAmountNumber: Number(rewardsAmountNumber) || 0,
    rewardsRatio: Number(rewardsRatio) || 0,
    referralId: String(referralId),
    referralName: String(referralName),
  });

  // 添加sponsorTX检查逻辑 - 修复3月24号后数据丢失问题
  const balanceChanges = ctx.transaction.balanceChanges;
  if (balanceChanges) {
    for (let i = 0; i < balanceChanges.length; i++) {
      const amount = balanceChanges[i].amount;
      ...
      if (
        ...
        console.log("Found sponsor owner in referral event:", ownerAddress);

        const events = ctx.transaction.events;
        console.log("Referral event transaction:", ctx.transaction.digest);
        ...
        if (events) {
          for (let j = 0; j < events.length; j++) {
            ...
            if (
              ...
              console.log("Emitting sponsorTX from referral event");
              ctx.eventLogger.emit("sponsorTX", {
                suiGas: Math.abs(Number(amount)) || 0,
                coinType: String(coinType),
                owner: String(ownerAddress),
